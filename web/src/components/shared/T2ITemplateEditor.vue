        ...
        <v-row no-gutters class="align-center">
          <v-col>
            <div class="text-caption text-grey">
              ...
              支持 jinja2 语法。可用变量：<code> text | safe </code>（要渲染的文本）, <code> version </code>（MindBot 版本）
            </div>
          </v-col>
          <v-col cols="auto">
            <v-btn
...
const loadTemplate = async () => {
  loading.value = true
  try {
    const response = await axios.get('/api/config/mindbot/t2i-template/get')
...
const saveTemplate = async () => {
  ...
  try {
    const response = await axios.post('/api/config/mindbot/t2i-template/save', {
...
const confirmReset = async () => {
  ...
  try {
    const response = await axios.delete('/api/config/mindbot/t2i-template/delete')
    if (response.data.status === 'ok') {
