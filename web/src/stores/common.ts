...
export const useCommonStore = defineStore({
  ...
  state: () => ({
    ...
    log_cache: [] as LogEntry[],
    sse_connected: false,

    log_cache_max_len: 1000,
    startTime: -1,

    pluginMarketData: [] as PluginMarketItem[],
  }),
  actions: {
    async createEventSource() {
      ...
      fetch('/api/live-log', {
        ...
        const handleIncompleteLine = (line: string): LogEntry | null => {
          incompleteLine += line;
        ...
        const processStream = ({ done, value }: { done: boolean; value?: Uint8Array }): Promise<void> => {
          ...
          for (const line of lines) {
            ...
            if (line.startsWith('data:')) {
              ...
              let data_json: LogEntry | null = null;
              try {
