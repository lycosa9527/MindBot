<template>
          ...
          <v-tabs-window v-model="tab" class="config-tabs-window">
            <v-tabs-window-item v-for="(val, key, index) in metadata" v-show="index == tab" :key="index">
              <v-container fluid>
                <div v-for="(val2, key2, index2) in metadata[key]['metadata']" :key="key2">
                  ...
                  <MindBotConfigV4 :metadata="{ [key2]: metadata[key]['metadata'][key2] }" :iterable="config_data"
                    :metadataKey="key2">
                  </MindBotConfigV4>
                </div>
              </v-container>
            ...
            <div style="margin-left: 16px; padding-bottom: 16px">
              <small>{{ tm('help.helpPrefix') }}
                <a href="https://github.com/lycosa9527/MindBot" target="_blank">{{ tm('help.documentation') }}</a>
                {{ tm('help.helpMiddle') }}
  ...
  <v-dialog v-model="configManageDialog" max-width="800px">
    <v-card>
      ...
      <v-card-text>
        <small>MindBot 支持针对不同消息平台实例分别设置配置文件。默认会使用 `default` 配置。</small>
        <div class="mt-6 mb-4">
...
import MindBotConfigV4 from '@/components/shared/MindBotConfigV4.vue';
...
export default {
  ...
  methods: {
    ...
    updateConfig() {
      ...
      axios.post('/api/config/mindbot/update', postData).then((res) => {
    ...
    formatConflictMessage(conflictDetails) {
      ...
      message += '<br><small><strong>💡 说明：</strong> 您仍可创建此配置文件。MindBot 按配置文件创建顺序匹配规则，先创建的配置文件优先级更高。当多个配置文件的规则匹配同一个消息会话来源时，优先级最高的配置文件会生效（default 配置文件除外）。</small>';
