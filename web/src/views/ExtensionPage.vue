...
const trimExtensionName = () => {
  pluginMarketData.value.forEach(plugin => {
    if (plugin.name) {
      ...
      if (name.startsWith("mindbot_plugin_")) {
        plugin.trimmedName = name.substring(15);
      } else if (name.startsWith("mindbot_") || name.startsWith("mindbot-")) {
...
<template>
  <v-row>
    ...
    <v-col v-if="activeTab === 'market'" style="margin-bottom: 16px;" cols="12" md="12">
      <small><a href="https://github.com/lycosa9527/MindBot">{{ tm('market.devDocs') }}</a></small> |
      <small> <a href="https://github.com/lycosa9527/MindBot">{{ tm('market.submitRepo') }}</a></small>
    </v-col>
  </v-row>
  ...
  <v-dialog v-model="configDialog" width="1000">
    <v-card>
      ...
      <v-card-text>
        <MindBotConfig v-if="extension_config.metadata" :metadata="extension_config.metadata"
